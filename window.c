...
+*/
{
  ...
  newwp = window_new ();
  newwp->fwidth = cur_wp->fwidth / 2 + cur_wp->fwidth % 2;
  newwp->ewidth = newwp->fwidth - 1;
  newwp->fheight = cur_wp->fheight;
  newwp->eheight = newwp->fheight - 1;

  cur_wp->ewidth = cur_wp->fwidth - newwp->fwidth;
  cur_wp->fwidth = cur_wp->fwidth - newwp->fwidth;
  cur_wp->fheight = cur_wp->fheight;
  cur_wp->eheight = cur_wp->fheight - 1;

  newwp->x_pos = cur_wp->x_pos + cur_wp->fwidth;
  newwp->y_pos = cur_wp->y_pos;

  /* Debug stuff - shows the data on terminal */
  printf ("original window: x=%d y=%d w=%d new window: x=%d y=%d w=%d",
	  cur_wp->x_pos,
	  cur_wp->y_pos,
	  cur_wp->fwidth,
	  newwp->x_pos,
...
DEFUN ("split-window-vertically", split_window_vertically)
/*+
Split current window into two windows, one above the other.
Both windows display the same buffer now current.
+*/
{
  ...
  newwp->x_pos = cur_wp->x_pos;
  newwp->y_pos = cur_wp->y_pos + cur_wp->fheight;
...
create_first_window (void)
{
  ...
  /* Sets the initial coordinates of the window */
  wp->x_pos = 0;
  wp->y_pos = 0;
